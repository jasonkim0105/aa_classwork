...
function pathFinder(directories, targetFile) {
    if (directories.hasOwnProperty(targetFile)) return `/${targetFile}`

    let path = null;
    Object.keys(directories).forEach(directory => {
        if (directory[0] === '/') {
            const result = directory + pathFinder(directories[directory], targetFile);
            if (result.includes(targetFile)) path = result;
        }
    })
    return path;
}
